...
class JSSTreplyModel {
    ...
    function storeReplies($data) {
        ...
        if (jssupportticket::$_db->last_error == null) {
            ...
            JSSTmessage::setMessage(__('REPLY_HAS_BEEN_STORED', 'js-support-ticket'), 'updated');
            $messagetype = __('SUCCESSFULLY', 'js-support-ticket');
        }else {
            JSSTincluder::getJSModel('systemerror')->addSystemError(); // if there is an error add it to system errorrs
            JSSTmessage::setMessage(__('REPLY_HAS_NOT_BEEN_STORED', 'js-support-ticket'), 'error');
        ...
        if ($sendEmail == true) {
            if (is_admin()) {
                JSSTincluder::getJSModel('email')->sendMail(1, 4, $data['ticketid']); // Mailfor, Reply Ticket
            } else {
                JSSTincluder::getJSModel('email')->sendMail(1, 5, $data['ticketid']); // Mailfor, Reply Ticket
            }
    ...
    function removeReplies($id) {
        ...
        if (jssupportticket::$_db->last_error == null) {
            JSSTmessage::setMessage(__('REPLY_HAS_BEEN_DELETED', 'js-support-ticket'), 'updated');
        } else {
            JSSTincluder::getJSModel('systemerror')->addSystemError(); // if there is an error add it to system errorrs
            JSSTmessage::setMessage(__('REPLY_HAS_NOT_BEEN_DELETED', 'js-support-ticket'), 'error');
        }
        return;
    }
