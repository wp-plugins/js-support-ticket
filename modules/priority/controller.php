...
class JSSTpriorityController {
    ...
    function handleRequest() {
        $layout = JSSTrequest::getLayout('layout', null, 'priorities');
        if (self::canaddfile()) {
            switch ($layout) {
    ...
    static function savepriority() {
        ...
        if (is_admin()) {
            $url = admin_url("admin.php?page=priority&layout=priorities");
        } else {
    ...
    static function makedefault() {
        ...
        $url = "admin.php?page=priority&layout=priorities";
        if ($pagenum)
            $url .= '&pagenum=' . $pagenum;
        wp_redirect($url);
        exit;
