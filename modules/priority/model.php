...
class JSSTpriorityModel {
    ...
    function getPriorities() {
        ...
        $query .= $inquery;
        $query .= " ORDER BY priority.ordering ASC LIMIT " . JSSTpagination::$_offset . ", " . JSSTpagination::$_limit;
    ...
    function storePriority($data) {
        if (!$this->validatePriority($data['priority'], $data['id'])) {
            JSSTmessage::setMessage(__('PRIORITY_TITLE_ALREADY_EXIST', 'js-support-ticket'), 'error');
            return;
        }
        ...
        if (jssupportticket::$_db->last_error == null) {
            JSSTmessage::setMessage(__('PRIORITY_HAS_BEEN_STORED', 'js-support-ticket'), 'updated');
        } else {
            JSSTincluder::getJSModel('systemerror')->addSystemError(); // if there is an error add it to system errorrs
            JSSTmessage::setMessage(__('PRIORITY_HAS_NOT_BEEN_STORED', 'js-support-ticket'), 'error');
        }
        return;
    }
    ...
    function removePriority($id) {
        ...
        if ($canremove == 1) {
            ...
            if (jssupportticket::$_db->last_error == null) {
                JSSTmessage::setMessage(__('PRIORITY_HAS_BEEN_DELETED', 'js-support-ticket'), 'updated');
            } else {
                JSSTincluder::getJSModel('systemerror')->addSystemError(); // if there is an error add it to system errorrs
                JSSTmessage::setMessage(__('PRIORITY_HAS_NOT_BEEN_DELETED', 'js-support-ticket'), 'error');
            }
        } elseif ($canremove == 2)
            JSSTmessage::setMessage(__('PRIORITY_IN_USE_CANNOT_DELETED', 'js-support-ticket'), 'error');
        elseif ($canremove == 3)
            JSSTmessage::setMessage(__('DEFAULT_PRIORITY_CANNOT_DELETE', 'js-support-ticket'), 'error');
    ...
    function makeDefault($id) {
        ...
        jssupportticket::$_db->query($query);
        if (jssupportticket::$_db->last_error == null) {
            JSSTmessage::setMessage(__('PRIORITY_HAS_BEEN_MAKE_DEFAULT', 'js-support-ticket'), 'updated');
        } else {
            JSSTincluder::getJSModel('systemerror')->addSystemError(); // if there is an error add it to system errorrs
            JSSTmessage::setMessage(__('PRIORITY_HAS_NOT_BEEN_MAKE_DEFAULT', 'js-support-ticket'), 'error');
        }
        return;
    }
    ...
    function setOrdering($id) {
        ...
        jssupportticket::$_db->query($query);
        if (jssupportticket::$_db->last_error == null) {
            JSSTmessage::setMessage(__('PRIORITY_ORDERING_HAS_BEEN_CHANGED', 'js-support-ticket'), 'updated');
        } else {
            JSSTincluder::getJSModel('systemerror')->addSystemError(); // if there is an error add it to system errorrs
            JSSTmessage::setMessage(__('PRIORITY_ORDERING_HAS_NOT_CHANGED', 'js-support-ticket'), 'error');
        }
        return;
    }
