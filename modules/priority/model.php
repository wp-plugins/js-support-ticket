...
class JSSTpriorityModel {
    ...
    function removePriority($id) {
        ...
        if ($canremove == 1) {
            ...
            JSSTmessage::setMessage(__('Priority in use cannot deleted', 'js-support-ticket'), 'error');
        elseif ($canremove == 3)
    ...
    function setOrdering($id) {
        ...
        jssupportticket::$_db->query($query);
        if (jssupportticket::$_db->last_error == null) {
            JSSTmessage::setMessage(__('Priority ordering has been changed', 'js-support-ticket'), 'updated');
        } else {
            JSSTincluder::getJSModel('systemerror')->addSystemError(); // if there is an error add it to system errorrs
            JSSTmessage::setMessage(__('Priority ordering has not changed', 'js-support-ticket'), 'error');
        }
        return;
    }
