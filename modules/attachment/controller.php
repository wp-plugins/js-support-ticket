...
class JSSTattachmentController {
    ...
    function handleRequest() {
        $layout = JSSTrequest::getLayout('layout', null, 'getattachments');
        if (self::canaddfile()) {
            switch ($layout) {
                case 'getattachments':
    ...
    static function saveattachments() {
        ...
        if (is_admin()) {
            $url = admin_url("admin.php?page=ticket&layout=ticketdetail&jssupportticketid=" . JSSTrequest::getVar('ticketid'));
        } else {
            $url = site_url("?page_id=" . jssupportticket::getPageid() . "&module=replies&layout=replies");
        }
        wp_redirect($url);
        exit;
    }
    ...
    static function deleteattachment() {
        ...
        if (is_admin()) {
            $url = admin_url("admin.php?page=ticket&layout=addticket&jssupportticketid=" . JSSTrequest::getVar('ticketid'));
        } else {
            $url = site_url("?page_id=" . jssupportticket::getPageid() . "&module=replies&layout=replies");
        }
        wp_redirect($url);
