...
class JSSTattachmentModel {
		        ...
		        if ($_FILES['filename']['name'][$i] != '') {
		            if ($_FILES['filename']['size'][$i] > 0) {
		                ...
		                if ($uploadfilesize > $filesize) { // file size error
		                    JSSTmessage::setMessage(__('FILE_SIZE_GREATER_THEN_DEFAULT_FILESIZE', 'js-support-ticket'), 'error');
		                    return;
		                }
		                ...
		                if ($result == 'N') {//file extension error
		                    JSSTmessage::setMessage(__('FILE_EXTENSION_DOES_NOT_MATCH', 'js-support-ticket'), 'error');
		                    return;
		                }
    ...
    function removeAttachment($id) {
        ...
        if (jssupportticket::$_db->last_error == null) {
            ...
            //$files = glob($path.'/*.*');
            //array_map('unlink', $files); // delete all file in the direcoty
            JSSTmessage::setMessage(__('ATTACHMENT_HAS_BEEN_REMOVED', 'js-support-ticket'), 'updated');
        } else {
            JSSTincluder::getJSModel('systemerror')->addSystemError();
            JSSTmessage::setMessage(__('ATTACHMENT_HAS_NOT_BEEN_REMOVED', 'js-support-ticket'), 'error');
        }
    }
