...
class JSSTsystemerrorController {
    ...
    function handleRequest() {
        $layout = JSSTrequest::getLayout('layout', null, 'systemerrors');
        if (self::canaddfile()) {
            switch ($layout) {
    ...
    static function savesystemerror() {
        ...
        if (is_admin()) {
            $url = admin_url("admin.php?page=systemerror&layout=systemerrors");
        } else {
            $url = site_url("?page_id=" . jssupportticket::getPageid() . "&module=systemerror&layout=systemerrors");
        }
        wp_redirect($url);
        exit;
    }
