...
class JSSTticketModel {
    ...
    function combineOrSingleSearch(){
        ...
        if($ticketkeys){
            ...
            if($inquery == '') jssupportticket::$_data['filter']['combinesearch'] = false;
            else jssupportticket::$_data['filter']['combinesearch'] = true;
        }
        return $inquery;
    }
    ...
    function storeTickets($data) {
        ...
        if (jssupportticket::$_db->last_error != null) {
            ...
            $sendEmail = false;
            JSSTmessage::setMessage(__('TICKET_HAS_NOT_BEEN_STORED', 'js-support-ticket'), 'error');
        } else {
            ...
            JSSTmessage::setMessage(__('TICKET_HAS_BEEN_STORED', 'js-support-ticket'), 'updated');
        }
    ...
    function removeTicket($id) {
        ...
        if ($this->canRemoveTicket($id)) {
            ...
            if (jssupportticket::$_db->last_error == null) {
                $messagetype = __('SUCCESSFULLY', 'js-support-ticket');
                JSSTmessage::setMessage(__('TICKET_HAS_BEEN_DELETED', 'js-support-ticket'), 'updated');
            } else {
                JSSTincluder::getJSModel('systemerror')->addSystemError(); // if there is an error add it to system errorrs
                JSSTmessage::setMessage(__('TICKET_HAS_NOT_BEEN_DELETED', 'js-support-ticket'), 'error');
                $messagetype = __('ERROR', 'js-support-ticket');
                $sendEmail = false;
            }
            ...
            JSSTmessage::setMessage(__('TICKET_IN_USE_CANNOT_BE_DELETED', 'js-support-ticket'), 'error');
        }
    ...
    function closeTicket($id) {
        ...
        if(!$this->checkActionStatusSame($id,array('action'=>'closeticket'))){
            JSSTmessage::setMessage(__('TICKET_ALREADY_CLOSED','js-support-ticket'),'error');
            return;
        }
        $sendEmail = true;
        $date = date('Y-m-d H:i:s');
        ...
        jssupportticket::$_db->query($query);
        if (jssupportticket::$_db->last_error == null) {
            JSSTmessage::setMessage(__('TICKET_HAS_BEEN_CLOSED', 'js-support-ticket'), 'updated');
            $messagetype = __('SUCCESSFULLY', 'js-support-ticket');
        } else {
            JSSTincluder::getJSModel('systemerror')->addSystemError(); // if there is an error add it to system errorrs
            JSSTmessage::setMessage(__('TICKET_HAS_NOT_BEEN_CLOSED', 'js-support-ticket'), 'error');
            $messagetype = __('ERROR', 'js-support-ticket');
            $sendEmail = false;
        }
    ...
    function changeTicketPriority($id, $priorityid) {
        ...
        jssupportticket::$_db->query($query);
        if (jssupportticket::$_db->last_error == null) {
            JSSTmessage::setMessage(__('TICKET_PRIORITY_HAS_BEEN_CHANGED', 'js-support-ticket'), 'updated');
            $messagetype = __('SUCCESSFULLY', 'js-support-ticket');
        } else {
            JSSTincluder::getJSModel('systemerror')->addSystemError(); // if there is an error add it to system errorrs
            JSSTmessage::setMessage(__('TICKET_PRIORITY_HAS_NOT_BEEN_CHANGED', 'js-support-ticket'), 'error');
            $messagetype = __('ERROR', 'js-support-ticket');
            $sendEmail = false;
        }
        // Send Emails
    ...
    function reopenTicket($data) {
        ...
        if ($this->checkCanReopenTicket($ticketid)) {
            ...
            jssupportticket::$_db->query($query);
            if (jssupportticket::$_db->last_error == null) {
                JSSTmessage::setMessage(__('TICKET_HAS_BEEN_REOPENED', 'js-support-ticket'), 'updated');
                $messagetype = __('SUCCESSFULLY', 'js-support-ticket');
            } else {
                JSSTincluder::getJSModel('systemerror')->addSystemError(); // if there is an error add it to system errorrs
                JSSTmessage::setMessage(__('TICKET_HAS_NOT_BEEN_REOPENED', 'js-support-ticket'), 'error');
                $messagetype = __('ERROR', 'js-support-ticket');
                $sendEmail = false;
            }
