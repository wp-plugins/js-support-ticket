	...
	function storeConfiguration($data){
		...
		foreach($data AS $key => $value){
			...
			if($key == 'pagination_default_page_size'){
				if($value < 3){
					JSSTmessage::setMessage(__('PAGINATION_DEFAULT_PAGE_SIZE_NOT_SAVED','js-support-ticket'),'updated');
					continue;
				}
			}
		...
		if($notsave == false){
			JSSTmessage::setMessage(__('CONFIGURATION_HAS_BEEN_STORED','js-support-ticket'),'updated');
		}else{
			JSSTmessage::setMessage(__('CONFIGURATION_NOT_HAS_BEEN_STORED','js-support-ticket'),'error');
		}
		return;
	}
