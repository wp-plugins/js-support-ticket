...
class JSSTemailModel {
                ...
                switch ($action) {
                    case 1: // New Ticket Created
                        ...
                        if (jssupportticket::$_config['new_ticket_mail_to_admin'] == 1) {
                            ...
                            $link = "<a href=" . admin_url("admin.php?page=ticket&layout=ticketdetail&jssupportticketid=" . $id) . ">" . __('TICKET_DETAIL', 'js-support-ticket') . "</a>";
                        ...
                        $msgSubject = $template->subject;
                        $msgBody = $template->body;
                        $link = "<a href=" . site_url("?page_id=" . $pageid . "&module=ticket&layout=ticketdetail&jssupportticketid=" . $id) . ">" . __('TICKET_DETAIL', 'js-support-ticket') . "</a>";
                        $matcharray['{TICKETURL}'] = $link;
                        $this->replaceMatches($msgSubject, $matcharray);
                        $this->replaceMatches($msgBody, $matcharray);
    ...
    function storeEmail($data) {
        ...
        if (jssupportticket::$_db->last_error == null) {
            JSSTmessage::setMessage(__('EMAIL_HAS_BEEN_STORED', 'js-support-ticket'), 'updated');
        } else {
            JSSTincluder::getJSModel('systemerror')->addSystemError(); // if there is an error add it to system errorrs
            JSSTmessage::setMessage(__('EMAIL_HAS_NOT_BEEN_STORED', 'js-support-ticket'), 'error');
        }
        return;
    }
    ...
    function removeEmail($id) {
        ...
        if ($this->canRemoveEmail($id)) {
            ...
            if (jssupportticket::$_db->last_error == null) {
                JSSTmessage::setMessage(__('EMAIL_HAS_BEEN_DELETED', 'js-support-ticket'), 'updated');
            } else {
                JSSTincluder::getJSModel('systemerror')->addSystemError(); // if there is an error add it to system errorrs
                JSSTmessage::setMessage(__('EMAIL_HAS_NOT_BEEN_DELETED', 'js-support-ticket'), 'error');
            }
        } else {
            JSSTmessage::setMessage(__('EMAIL_IN_USE_CANNOT_DELETED', 'js-support-ticket'), 'error');
        }
        return;
    }
